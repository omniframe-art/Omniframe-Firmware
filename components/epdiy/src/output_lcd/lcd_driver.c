...
void IRAM_ATTR epd_lcd_init(const LcdEpdConfig_t* config, int display_width, int display_height) {
    ...
    // Log the pin number being configured
    //ESP_LOGI("EPD_LCD_INIT", "Configuring GPIO for VSYNC on pin %d", lcd.config.bus.stv);
    ...
    // Configure the GPIO
    esp_err_t err = gpio_config(&vsync_gpio_conf);

    if (err != ESP_OK) {
        ESP_LOGE("EPD_LCD_INIT", "Failed to configure GPIO for VSYNC: %s", esp_err_to_name(err));
        return;
    }
